...
## NAS / Remote Deployment Issues

### `export-token` says HTTPS is required

`msgvault export-token` enforces HTTPS by default for token upload.

- Use `https://nas-host:8080` when possible.
- If you must use plain HTTP on a trusted network, add `--allow-insecure`.
- You can persist this behavior in `~/.msgvault/config.toml` via `[remote] allow_insecure = true` when using remote mode.
...
3. Ensure the NAS endpoint is reachable from the local machine.
...
Ask for help on the [msgvault Discord server](https://discord.gg/fDnmxB8Wkq) or [open an issue on GitHub](https://github.com/wesm/msgvault/issues).
