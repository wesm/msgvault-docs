...
url = "https://remote-host.example.com:8080"
api_key = "remote-api-key"
# Set to true only for trusted HTTP-only networks
...
| Key | Default | Description |
|---|---|---|
| `url` | — | Remote API base URL (e.g. `https://myserver.local:8080`) |
...
| `MSGVAULT_REMOTE_URL` | Fallback remote URL for `export-token` when not in config |
| `MSGVAULT_REMOTE_API_KEY` | Fallback remote API key for `export-token` |
