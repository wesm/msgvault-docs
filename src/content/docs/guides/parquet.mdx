...
msgvault solves this with a denormalized Parquet cache queried by an embedded DuckDB engine. The result is aggregate queries **hundreds of times faster**: what takes seconds in SQLite returns in milliseconds from Parquet. The analytics layer has a tiny footprint compared to the full SQLite database with message bodies, so it loads instantly and stays out of your way.
