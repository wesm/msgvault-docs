...
6. Add scopes based on your setup:
   - **Desktop (browser flow):** `https://www.googleapis.com/auth/gmail.modify`
   - **Headless (device flow):** `https://mail.google.com/` (required—device flow doesn't support granular scopes)
7. Save and continue through the remaining screens
8. Under **Test users**, add all Gmail addresses you want to sync

:::note
For desktop/browser flow, the `gmail.modify` scope enables deletion features while sync operations remain read-only. For headless/device flow, you must use the full `mail.google.com` scope as Google doesn't support granular scopes with device authorization.
:::
...
:::note[Headless OAuth Scope]
Google's device code flow only supports the full `mail.google.com` scope—it does not support granular scopes like `gmail.readonly` or `gmail.modify`. This means headless authorization grants full Gmail access, which is broader than the standard browser flow. The token works identically for all msgvault operations including deletion.
:::
