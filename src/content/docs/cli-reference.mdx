...
Use this for headless deployments like NAS containers that cannot run a browser flow.
...
Run the first-run setup wizard for OAuth and optional remote NAS deployment.

```bash
msgvault setup
```

If configured for a remote NAS, this command generates `<MSGVAULT_HOME>/nas-bundle` with:
