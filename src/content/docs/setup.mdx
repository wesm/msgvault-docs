...
:::caution[Windows]
There are a number of known issues on Windows that are being actively resolved. Fixes are coming within the next few days. See [Troubleshooting > Windows](/troubleshooting/#windows) for current status.
:::
...
:::tip
The `.msgvault` directory is created automatically the first time you run any msgvault command. If you're unsure of the exact path, run `msgvault add-account you@gmail.com` and the error message will show you where to create the config file.
:::

To store data on a different drive or location, set the `MSGVAULT_HOME` environment variable:
