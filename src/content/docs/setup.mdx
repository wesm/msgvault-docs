...
Requires Go 1.21+ and a C compiler (GCC or Clang). CGO is required because msgvault uses `mattn/go-sqlite3` which compiles SQLite as a C extension for full-text search (FTS5).
