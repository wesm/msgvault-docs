...
:::caution[Windows]
There are a number of known issues on Windows that are being actively resolved. Fixes are coming within the next few days. In the meantime, download Windows packages directly from [GitHub Releases](https://github.com/wesm/msgvault/releases) and see [Troubleshooting > Windows](/troubleshooting/#windows) for current status.
:::
