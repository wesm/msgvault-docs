...
msgvault syncs your Gmail to a local SQLite database, the only step that touches the network. You can also import local MBOX archives and Apple Mail exports. Everything else (search, analytics, the TUI, and the MCP server) runs entirely offline against SQLite, Parquet, and local attachment files.
