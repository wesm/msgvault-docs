...
If you have the [MCP server](/usage/chat/) configured, you can ask an AI assistant to stage messages for deletion using natural language:

- *"Stage all emails from linkedin.com for deletion"*
- *"Stage promotional emails from before 2023 for deletion"*

The AI uses the `stage_deletion` tool to create a deletion manifest. Nothing is deleted until you explicitly run `msgvault delete-staged` from the CLI.
