...
The TUI currently requires a local database — it does not support [remote servers](/guides/remote-deployment/). Remote TUI support is planned for a future release. For now, use the CLI commands (`search`, `stats`, etc.) which work in remote mode, or SSH into the remote host and run `msgvault tui` there.
